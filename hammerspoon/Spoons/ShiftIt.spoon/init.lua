...
local obj = {
  hs = hs,
  gap = 8  -- Added global gap variable
}
obj.__index = obj
...
local function applyGap(unit, screen)
  local gap = obj.gap / screen:frame().w  -- Convert pixels to relative units
  return {
    x = unit.x + gap,
    y = unit.y + gap,
    w = unit.w - (2 * gap),
...
local units = {
  left  = function(x, _) return applyGap({ x = 0.00, y = 0.00, w = x / 100, h = 1.00 }, hs.window.focusedWindow():screen()) end,
  right = function(x, _) return applyGap({ x = 1 - (x / 100), y = 0.00, w = x / 100, h = 1.00 }, hs.window.focusedWindow():screen()) end,
  top   = function(_, y) return applyGap({ x = 0.00, y = 0.00, w = 1.00, h = y / 100 }, hs.window.focusedWindow():screen()) end,
  bot   = function(_, y) return applyGap({ x = 0.00, y = 1 - (y / 100), w = 1.00, h = y / 100 }, hs.window.focusedWindow():screen()) end,

  upleft   = function(x, y) return applyGap({ x = 0.00, y = 0.00, w = x / 100, h = y / 100 }, hs.window.focusedWindow():screen()) end,
  upright  = function(x, y) return applyGap({ x = 1 - (x / 100), y = 0.00, w = x / 100, h = y / 100 }, hs.window.focusedWindow():screen()) end,
  botleft  = function(x, y) return applyGap({ x = 0.00, y = 1 - (y / 100), w = x / 100, h = y / 100 }, hs.window.focusedWindow():screen()) end,
  botright = function(x, y) return applyGap({ x = 1 - (x / 100), y = 1 - (y / 100), w = x / 100, h = y / 100 }, hs.window.focusedWindow():screen()) end,

  maximum = function() return applyGap({ x = 0.00, y = 0.00, w = 1.00, h = 1.00 }, hs.window.focusedWindow():screen()) end,
  almostMaximum = function() return applyGap({ x = 0.075, y = 0.075, w = 0.85, h = 0.85 }, hs.window.focusedWindow():screen()) end,
}
...
function obj:resizeWindowInSteps(increment)
  ...
  local wStep = math.floor(screen.w / 36)  -- Changed step size to 1/36
  local hStep = math.floor(screen.h / 36)  -- Changed step size to 1/36
...
function obj:almostMaximum()
  latestMove.direction = 'almostMaximum'
  self:move(units.almostMaximum)
end
...
function obj:bindHotkeys(mapping)
  ...
  self.hs.hotkey.bind(self.mapping.almostMaximum[1], self.mapping.almostMaximum[2], function() self:almostMaximum() end)
